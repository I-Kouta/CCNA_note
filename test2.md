...
<img width="700" alt="" src="./images/test2/ホストB.png">
...
<img width="900" alt="" src="./images/test2/問題1.png">
...
<img width="900" alt="" src="./images/test2/問題2.png">
...
---
---
URL:https://network00.net/ip5/

---
・チーム数・作業者数が増えてきたためネットワークを4つに分割する。ネットワークアドレスは172.16.9.0 / 24。各チームは最大でも以下の通り。DBチーム:60名、ネットチーム:30名、テストチーム:30名、事務処理チーム:30名

DBチーム:  
サブネットマスク:255.255.255.192  
ネットワークアドレス:172.16.9.64  
ブロードキャストアドレス:172.16.9.127  
割り当てられるアドレス範囲:172.16.9.65 ~ 172.16.9.126

ネットチーム:  
サブネットマスク:255.255.255.224  
ネットワークアドレス:172.16.9.128  
ブロードキャストアドレス:172.16.9.159  
割り当てられるアドレス範囲:172.16.9.129 ~ 172.16.9.158

テストチーム:  
サブネットマスク:255.255.255.224  
ネットワークアドレス:172.16.9.160  
ブロードキャストアドレス:172.16.9.172.16.9.191  
割り当てられるアドレス範囲:172.16.9.161 ~ 172.16.9.190

事務処理チーム:  
サブネットマスク:255.255.255.224  
ネットワークアドレス:172.16.9.192  
ブロードキャストアドレス:172.16.9.223  
割り当てられるアドレス範囲:172.16.9.193 ~ 172.16.9.222

DBチームはサブネット化は2ビットのみ。以降はサブネット化は3ビット

---
・協力会社毎にネットワークを分ける。そこでサブネット7つに分割をすることにした。ネットワークアドレスは10.10.1.0 / 24。ロンドンシステム:13名、上海システム:13名、ソウルシステム:13名、ロサンゼルスシステム:13名、東京システム:13名、沖縄システム:13名、八十八電気:40名

ロンドンシステム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.16  
ブロードキャストアドレス:10.10.1.31  
割り当てられるアドレス範囲:10.10.1.17 ~ 10.10.1.30

上海システム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.32  
ブロードキャストアドレス:10.10.1.47  
割り当てられるアドレス範囲:10.10.1.33 ~ 10.10.1.46

ソウルシステム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.48  
ブロードキャストアドレス:10.10.1.63  
割り当てられるアドレス範囲:10.10.1.49 ~ 10.10.1.62

ロサンゼルスシステム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.64  
ブロードキャストアドレス:10.10.1.79  
割り当てられるアドレス範囲:10.10.1.65 ~ 10.10.1.78

東京システム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.80  
ブロードキャストアドレス:10.10.1.95  
割り当てられるアドレス範囲:10.10.1.81 ~ 10.10.1.94

沖縄システム:  
サブネットマスク:255.255.255.240  
ネットワークアドレス:10.10.1.96  
ブロードキャストアドレス:10.10.1.111  
割り当てられるアドレス範囲:10.10.1.97 ~ 10.10.1.110

八十八電気:  
サブネットマスク:255.255.255.192  
ネットワークアドレス:10.10.1.128  
ブロードキャストアドレス:10.10.1.191  
割り当てられるアドレス範囲:10.10.1.129 ~ 10.10.1.190

16分割する想定で4ビットサブネット化する。40名を補うには2の5乗だと32で足りないため、サブネット化は2ビットに制限する必要がある
