...
このコマンドで、Telnet接続を確立したホストのリストが表示される。また、`show users`コマンドを使うと、現在アクティブなEXECセッションとしてコンソールからの接続ユーザとTelnetからの接続セッションを同時に確認できる。conだとローカルコンソール、vtyだとリモート接続。ユーザが複数存在する場合は*が現在の端末セッションユーザを示す。
