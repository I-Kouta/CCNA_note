...
> `デフォルトゲートウェイ`:どこにデータを届ければ良いか分からないときの取り敢えずの送り先(ネットワークの出入口)。デフォルトゲートウェイに送られたデータは別のネットワークに流れていく。ゲートウェイは、規格の違うネットワークを中継する機械。ルータがこの役割をすることが多く、ルータのIPアドレスを指定する。自身が所属するネットワーク以外のネットワーク宛にパケットを送信するときに必ず通過する
